...
export async function createInvocationDocument(
  input: InvocationInput,
  options?: { accessToken?: string }
) {
  ...
  if (invocation.meta && invocation.meta["user"])
    websocket.emit(
      `invocation:${invocation.meta["user"]}@${invocation.version}`,
      invocation
    );
  if (options?.accessToken)
    websocket.emit(
      `invocation:${options.accessToken}@${invocation.version}`,
      invocation
    );
...
export async function findInvocations(
  query: FilterQuery<InvocationDocument>,
